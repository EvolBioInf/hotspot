...
void tabSetFieldSep(char *c) { fieldsep[0] = c[0]; }

/* tabGetPlainLine: get line without splitting in background */
char *tabGetPlainLine(FILE *fin) { return getLine(fin); }
...
static char *getLine(FILE *fin) {
	...
	/*   line[maxline-1] = '\n'; */
	/*   while((line=fgets(line,maxline,fin)) != NULL){ */
	/*     if(line[maxline-1] != '\n'){ */
	/*       p = maxline; */
	/*       maxline *= 2; */
...
static char *advquoted(char *p) {
	...
	for (i = 0, j = 0; p[j] != '\0'; i++, j++) {
		if (p[j] == '"' &&
		    p[++j] != '"') { /* copy up to next separator or \0 */
...
int tabNfield(void) { return nfield; }
